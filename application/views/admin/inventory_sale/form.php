...
<h5 class="font-20 mt-15 mb-1"><?php if($id<0){echo "Save";}else { echo "Update";} echo " "; echo str_replace('_',' ','Inventory_sale'); ?></h5>
<!--Form to save data-->
<?php echo form_open_multipart('admin/inventory_sale/save/'.$inventory_sale['id'],array("class"=>"form-horizontal")); ?>
<div class="card">
	<div class="card-body">
		<div class="form-group">
			...
			<div class="col-md-8">
				<input type="text" name="created_on" id="created_on"
					value="<?php echo ($this->input->post('created_on') ? $this->input->post('created_on') : $inventory_sale['created_on']); ?>"
		...
		<div class="form-group">
			...
			<div class="col-md-8">
				<input type="text" name="modified_on" id="modified_on"
					value="<?php echo ($this->input->post('modified_on') ? $this->input->post('modified_on') : $inventory_sale['modified_on']); ?>"
					class="form-control-static datepicker" />
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-md-8">
				<input type="text" name="status"
					value="<?php echo ($this->input->post('status') ? $this->input->post('status') : $inventory_sale['status']); ?>"
        ...
        ?> 
          <select name="crm_billingaddress_id"
            ...
            <option value="<?=$dataArr[$i]['id']?>"
					<?php if($inventory_sale['crm_billingaddress_id']==$dataArr[$i]['id']){ echo "selected";} ?>><?=$dataArr[$i]['address1']?></option> 
            <?php
            }
            ?> 
          </select>
        ...
        ?> 
          <select name="created_by_users_id" id="created_by_users_id"
            ...
            for ($i = 0; $i < count($dataArr); $i ++) {
                ?> 
            <option value="<?=$dataArr[$i]['id']?>"
					<?php if($inventory_sale['created_by_users_id']==$dataArr[$i]['id']){ echo "selected";} ?>><?=$dataArr[$i]['email']?></option> 
            <?php
            }
            ?> 
          </select>
        ...
        ?> 
          <select name="crm_customer_id" id="crm_customer_id"
            ...
            for ($i = 0; $i < count($dataArr); $i ++) {
                ?> 
            <option value="<?=$dataArr[$i]['id']?>"
					<?php if($inventory_sale['crm_customer_id']==$dataArr[$i]['id']){ echo "selected";} ?>><?=$dataArr[$i]['first_name']?></option> 
            <?php
            }
            ?> 
          </select>
        ...
        ?> 
          <select name="modified_by_users_id" id="modified_by_users_id"
            ...
            for ($i = 0; $i < count($dataArr); $i ++) {
                ?> 
            <option value="<?=$dataArr[$i]['id']?>"
					<?php if($inventory_sale['modified_by_users_id']==$dataArr[$i]['id']){ echo "selected";} ?>><?=$dataArr[$i]['email']?></option> 
            <?php
            }
            ?> 
          </select>
		...
		<div class="form-group">
			<label for="Crm Supplier" class="col-md-4 control-label">Crm Supplier</label>
			<div class="col-md-8"> 
          <?php
        $this->CI = & get_instance();
        $this->CI->load->database();
        $this->CI->load->model('Crm_supplier_model');
        $dataArr = $this->CI->Crm_supplier_model->get_all_crm_supplier();
        ?> 
          <select name="crm_supplier_id" id="crm_supplier_id"
					class="form-control" />
				<option value="">--Select--</option> 
            <?php
            for ($i = 0; $i < count($dataArr); $i ++) {
                ?> 
            <option value="<?=$dataArr[$i]['id']?>"
					<?php if($inventory_sale['crm_supplier_id']==$dataArr[$i]['id']){ echo "selected";} ?>><?=$dataArr[$i]['first_name']?></option> 
            <?php
            }
            ?> 
          </select>
        ...
        ?> 
          <select name="crm_shippingaddress_id"
            ...
            for ($i = 0; $i < count($dataArr); $i ++) {
                ?> 
            <option value="<?=$dataArr[$i]['id']?>"
					<?php if($inventory_sale['crm_shippingaddress_id']==$dataArr[$i]['id']){ echo "selected";} ?>><?=$dataArr[$i]['address1']?></option> 
            <?php
            }
            ?> 
          </select>
...
<div class="form-group">
	<div class="col-sm-offset-4 col-sm-8">
		<button type="submit" class="btn btn-success"><?php if(empty($inventory_sale['id'])){?>Save<?php }else{?>Update<?php } ?></button>
	</div>
</div>
<?php echo form_close(); ?>
